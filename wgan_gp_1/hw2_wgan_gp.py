...
def eval_generator(load_mdl):
    ...
    if(load_mdl):
        generator.load_state_dict(torch.load('./mdl_state/gemdl_save_b64'))
        #generator.load_state_dict(torch.load('./mdl_state/gemdl_save_600'))
    generator.eval()
    generator.cuda()
    centerCrop = transforms.CenterCrop((out_size,out_size))
    ...
    for ii in range(grid_iters):
        ...
        g_imgs = g_imgs.cpu()
        g_imgs = g_imgs*0.9
        c_imgs = np.ones((img_batch, out_size, out_size, 3))
        for jj in range(img_batch):
            g_imgs[jj] = unorm(g_imgs[jj])
            gfake = transforms.ToPILImage()(g_imgs[jj])
            gcrop = transforms.CenterCrop(out_size)(gfake)

            c_imgs[jj] = np.array(gcrop.convert('RGB'))
        output_fig(c_imgs[:9], file_name="./fake_imgs/{}_image".format(str.zfill(str(ii+1), 3)))
    print('\n [INFO] Finished Gen Fake ')
###################################################

#check_set()
#train_set(load_mdl=False, save_mdl=True, save_loss_csv=True)
